...
insert into goods values(GOODS_SEQ.nextval, 'bbb','가방',45000,1,450000,sysdate);
...
--select class, sum(score) from testclass group by class
                                     --having sum(score) > 20;
select M.m_id,M.m_name, M.m_addr,sum(g.g_price_sum) from member M inner join goods G on M.m_id=G.g_id group by  M.m_id,M.m_id,M.m_name, M.m_addr;
--,M.m_name, M.m_addr,sum(g.g_price_sum)
